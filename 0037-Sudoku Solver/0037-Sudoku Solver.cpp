...
public:
    void solveSudoku(vector<vector<char>>& board) {
        usedRows = vector<vector<bool>>(9, vector<bool>(9, false));
        usedCols = vector<vector<bool>>(9, vector<bool>(9, false));
        usedBlocks = vector<vector<bool>>(9, vector<bool>(9, false));
        for (int i = 0; i < 9; ++i)
        {
            for (int j = 0; j < 9; ++j)
            {
            ...
            for (int j = 0; j < 9; ++j)
            {
                ...
                for (char v = '1'; v <= '9'; ++v)
                {
                    ...
                    if (!usedRows[i][index] && !usedCols[index][j] && !usedBlocks[k][index])
                    {
                        ...
                        board[i][j] = '.';
                        usedRows[i][index] = usedCols[index][j] = usedBlocks[k][index] = false;
                    }
                }
                return false;
            }
